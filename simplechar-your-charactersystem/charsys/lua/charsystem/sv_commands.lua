...
local commands = {
    ["charname"] = function(ply, args)
        if not CharSystem.Config.AllowedNameChange[ply:GetUserGroup()] then
    ...
    ["setjob"] = function(ply, args)
        if not CharSystem.Config.AllowedJobChange[ply:GetUserGroup()] and not CharSystem.Config.AllowedJobChange[ply:SteamID()] then
            ply:ChatAddText(Color(198, 0, 0), CharSystem.Config.NoPermissions)
        ...
        for k, v in pairs(RPExtraTeams) do
            -- if the job name we need to find is equal to v.name k is the jobid
            if string.lower(jobnameCH) == string.lower(v.name) then
                NewJob = k
            end
        end
    ...
    ["chars"] = function(ply, args)
        if not CharSystem.Config.AllowedAdminMenu[ply:GetUserGroup()] and not CharSystem.Config.AllowedAdminMenu[ply:SteamID()] then
            ply:ChatAddText(Color(198, 0, 0), CharSystem.Config.NoPermissions)
    ...
    ["invite"] = function(ply, args)
        if CharSystem.Config.AllowedInvite[ply:GetUserGroup()] or CharSystem.Config.AllowedInvite[ply:SteamID()] then
            ...
            if not InvitedPlayerName or InvitedPlayerName == " " or InvitedPlayerName == "" then
                ply:ChatAddText(Color(198, 0, 0), string.Replace(CharSystem.Config.NoPlayerFound, "%name%", args[2]))
    ...
    ["jkick"] = function(ply, args)
        if not CharSystem.Config.PermissionsKick[ply:GetUserGroup()] and not CharSystem.Config.PermissionsKick[ply:SteamID()] then
            ply:ChatAddText(Color(198, 0, 0), CharSystem.Config.NoPermissions)
        else
    ...
    ["train"] = function(ply, args)
        if not CharSystem.Config.Faction1Ranks[ply:GetUserGroup()] and not ChasrSystem.Config.Faction2Ranks[ply:GetUserGroup()] and not ChasrSystem.Config.Faction3Ranks[ply:GetUserGroup()] then
...
hook.Add("PlayerSay", "CharacterSystemCommands", function(ply, text, teamchat)
    ...
    if not string.StartWith(args[1], cmdPrefix) then return end
